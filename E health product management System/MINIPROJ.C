...
/* function to center the given string horizontally */
...
{       char u;
	...
	printf("%s",ctime(&t));
	centerh("|=================================|",8,12);
	centerh("WELCOME TO",10,12);
	centerh("E-HEALTH",12,12);
	centerh("PRODUCT MANAGEMENT SYSTEM",14,12);
	centerh("|=================================|",16,12);
	centerh("""WE BELIEVE IN QUALITY""",18,12);
	gotoxy(20,22);
    ...
    do
{
	...
	i=0;
	//scanf("%s",&pword);
		if(strcmp(uname,"company")==0 && strcmp(pword,"1234")==0)
	{
	printf("\nWELCOME TO PRODUCT MANAGEMENT SYSTEM !!!! LOGIN IS SUCCESSFUL");
	printf("\n\n\n\t\t\t\tPress any key to continue...");
	getch();//holds the screen
		...
		do
			{
				...
				if(st.price<10)
				{
					printf("\n\tYou Cannot Enter the price limit [10].Re-Enter.");
				}
			}while(st.price<10);
				...
				do
			{
				...
				if(st.Qnt<1)
				{
					printf("\n\tEnter Quantity[1<] only.Re-Enter.");
				}
			}while(st.Qnt<1);
	...
	else
	{
		...
		while(fscanf(fp,"%s %s %i %i %i\n", st.productname,st.productcomp, &st.price, &st.productid,&st.Qnt)!=EOF)
		{
				...
				do
			{
				...
				if(st.Qnt<1)
				{
					printf("\n\tEnter New Quantity[1<] only.Re-Enter.");
				}
			}while(st.Qnt<1);
